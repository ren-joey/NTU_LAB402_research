...
class WindowAttention(nn.Module):
    ...
    Args:
        ...
        pretrained_window_size (tuple[int]): The Depth, height and width of the window in pre-training.
    """
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.,
        ...
        relative_coords[:, :, 1] *= 3 * self.window_size[1] - 1 # Created by myself
    ...
    def forward(self, x, mask=None):
        ...
        if self.q_bias is not None:
            qkv_bias = torch.cat((self.q_bias, torch.zeros_like(self.v_bias, requires_grad=False), self.v_bias)) # dim * 3
        qkv = F.linear(input=x, weight=self.qkv.weight, bias=qkv_bias) # B_, N, 3C
        ...
        relative_position_bias_table = self.cpb_mlp(self.relative_coords_table).view(-1, self.num_heads) # (2*Wd-1)*(2*Wh-1)*(2*Ww-1), nH
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=(4, 7, 7), shift_size=(0, 0, 0),
        ...
        if min(self.shift_size) > 0:
            ...
            mask_windows = window_partition(img_mask, self.window_size)  # nW, window_size, window_size, window_size, C
            mask_windows = mask_windows.view(-1, self.window_size[0] * self.window_size[1] * self.window_size[2])
            attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2) # nW*B*C, window_size^3, window_size^3
    ...
    def forward(self, x):
        ...
        # merge windows
        attn_windows = attn_windows.view(-1, self.window_size[0], self.window_size[1], self.window_size[2], C) # nW*B, window_size, window_size, window_size, C
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self, img_size=(128, 224, 224), patch_size=4, in_chans=1, num_classes=2,
                 embed_dim=8, depths=[2, 2, 6, 2], num_heads=[1, 2, 4, 8],
                 window_size=(4, 7, 7), mlp_ratio=4., qkv_bias=True,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
                 use_checkpoint=False, pretrained_window_sizes=[0, 0, 0, 0], 
                 len_of_clinical_features=150, **kwargs):
        super().__init__()
    ...
    def forward_features(self, x):
        x = self.patch_embed(x) # B Pd*Ph*Pw C
        if self.ape:
