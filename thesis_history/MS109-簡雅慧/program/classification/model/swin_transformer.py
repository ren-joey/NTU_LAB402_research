...
class InvertRes(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, stride=1, expand_ratio=4., act_layer=nn.GELU, reduction=4, wo_dp_conv=False, dp_first=False):
        
        #:param in_features: the input dimension
        #:param out_features: the output dimension. The input and output dimension should be the same.
        #:param stride: stride of the depth-wise convolution. #1
        #:param expand_ratio: expansion ratio of the hidden dimension.
        #:param act_layer: the activation function.
        #                relu: ReLU
        #                GELU
        #                hs: h_swish
        ...
        #:param wo_dp_conv: without depth-wise convolution. #False
        #:param dp_first: place depth-wise convolution as the first layer. #False
        ...
        layers.extend([
            nn.Conv3d(in_features, hidden_features, kernel_size=1, stride=1, padding=0, bias=False),
        ...
        if not wo_dp_conv:
            dp = [
                nn.Conv3d(hidden_features, hidden_features, kernel_size, stride, kernel_size // 2, groups=hidden_features, bias=False),
                nn.BatchNorm3d(hidden_features),
                act_layer()]
                #h_swish() if act.find('hs') >= 0 else nn.ReLU6(inplace=True)]
            if dp_first:
        ...
        if act.find('+') >= 0: #?
            attn = act.split('+')[1]
            if attn == 'se':
                layers.append(SELayer(hidden_features, reduction=reduction))
            elif attn.find('eca') >= 0:
                layers.append(ECALayer(hidden_features, sigmoid=attn == 'eca'))
            else:
        ...
        layers.extend([
            nn.Conv3d(hidden_features, out_features, kernel_size=1, stride=1, padding=0, bias=False),
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        super().__init__()
        self.dim = dim
        self.window_size = window_size  # Wd, Wh, Ww #8, 8, 8
        ...
        coords_h = torch.arange(self.window_size[1]) # tensor([ 0, 1, 2, 3, 4, 5, 6, 7])
        ...
        relative_coords[:, :, 0] *= (2 * self.window_size[1] - 1)**2
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=8, shift_size=0,
        ...
        self.invertres = InvertRes(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer)
        ...
        if self.shift_size > 0:
            ...
            mask_windows = mask_windows.view(-1, self.window_size * self.window_size * self.window_size) #nW, window_size*window_size*window_size
            attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
            attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0)) #nW, winodw_size**2, window_size**2, winodw_size**2
        else:
            attn_mask = None
    ...
    def forward(self, x):
        ...
        """
        ### Inverted Residual(CNN)
        x = shortcut + self.drop_path(x)
        x = self.norm2(x)
        # reshape to image
        x = x.transpose(1,2).view(B, C, D, H, W)
        x = self.invertres(x).flatten(2).transpose(1,2)
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        x = torch.cat([x0, x1, x2, x3], -1)  # B D H/2 W/2 8*C
    ...
    def flops(self):
        ...
        flops += (D) * (H // 2) * (W // 2) * 4 * self.dim * 2 * self.dim
        return flops
...
class BasicLayer(nn.Module):
    ...
    Args:
        ...
        norm_layer (nn.Module, optional): Normalization layer. Default: nn.LayerNorm
        downsample (nn.Module | None, optional): Downsample layer at the end of the layer. Default: None #patch merging or not
...
class PatchEmbed(nn.Module):
    ...
    Args:
        img_size (int): Image size.  Default: (16,128,128). #(32,224,224)
        dpatch_size (int): Patch token size for depth. Default: 2.
        patch_size (int): Patch token size. Default: 2 # 4.
        in_chans (int): Number of input image channels. Default: 3.
        embed_dim (int): Number of linear projection output channels. Default: 96.
        norm_layer (nn.Module, optional): Normalization layer. Default: None
    """

    def __init__(self, img_size=(16,128,128), dpatch_size=2, patch_size=2, in_chans=3, embed_dim=96, norm_layer=None):
        super().__init__()
        #img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
        patches_resolution = [img_size[0] // dpatch_size, img_size[1] // patch_size[0], img_size[2] // patch_size[1]]
        ...
        self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=(dpatch_size, patch_size[0], patch_size[1]), stride=(dpatch_size, patch_size[0], patch_size[1]))
    ...
    def forward(self, x):
        ...
        x = self.proj(x).flatten(2).transpose(1, 2)  # B Pd*Ph*Pw C #channel_last
    ...
    def flops(self):
        ...
        flops = Do * Ho * Wo * self.embed_dim * self.in_chans * (self.dpatch_size * self.patch_size[0] * self.patch_size[1])
        if self.norm is not None:
...
class SwinTransformer(nn.Module):
    ...
    Args:
        img_size (int | tuple(int)): Input image size. Default (16, 128, 128) #(32, 224, 224)
        dpatch_size (int): patch size for depth. Default:2
        patch_size (int | tuple(int)): Patch size. Default: 2 # 4
        in_chans (int): Number of input image channels. Default: 3
        num_classes (int): Number of classes for classification head. Default: 1 #1000
        embed_dim (int): Patch embedding dimension. Default: 96
        depths (tuple(int)): Depth of each Swin Transformer layer.
        num_heads (tuple(int)): Number of attention heads in different layers.
        window_size (int): Window size. Default: 8 #7
    ...
    def __init__(self, img_size=(16, 128, 128), dpatch_size=2, patch_size=2, in_chans=3, num_classes=1,
        ...
        self.num_features = int(embed_dim * 2 ** (self.num_layers - 1)) # number of features after the final stage
        self.mlp_ratio = mlp_ratio

        # split image into non-overlapping patches
        self.patch_embed = PatchEmbed(
            img_size=img_size, dpatch_size=dpatch_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               input_resolution=(patches_resolution[0], # // (2 ** i_layer if (i_layer==0) or (i_layer==1) else 1),
    ...
    def forward_features(self, x):
        x = self.patch_embed(x) # channel last
        if self.ape:
